...
List* setup_environment()
{
    ...
    append_primitive_procedure(env, "EXIT", 1, &primitive_exit);
...
Value* primitive_greater(List* arguments)
{
    List lst;
    lst.length = 2;
    lst.first = arguments->last;
    lst.last = arguments->first;
    return primitive_less(&lst);
}

Value* primitive_lesseq(List* arguments)
{
    Value* result = primitive_less(arguments);
    if (result->type == TYPE_ERROR) return result;
    if (!strcmp((char*)result->data, "T")) return result;
    return primitive_eq(arguments);
}

Value* primitive_greatereq(List* arguments)
{
    Value* result = primitive_greater(arguments);
    if (result->type == TYPE_ERROR) return result;
    if (!strcmp((char*)result->data, "T")) return result;
    return primitive_eq(arguments);
}
...
Value* primitive_apply(List* arguments)
{
    ...
    env.length = 0; env.first = NULL; env.last = NULL;
...
Value* primitive_type(List* arguments)
{
    ...
    return alloc_value(TYPE_SYMBOL, (char*)type_names[val->type]);
}

Value* primitive_exit(List* arguments)
{
    Value* val = arguments->first->value;

    exit(val->type == TYPE_INTEGER ? *(int*)val->data : 0);
    return val;
}
