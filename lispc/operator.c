...
Value* apply_operator(Operator* operator, List* arguments, List* environment)
{
    ...
    evaluated_args.length = 0; evaluated_args.first = NULL; evaluated_args.last = NULL;
...
Value* operator_lambda(List* arguments, List* environment)
{
    ...
    arglst = (List*)arglist->data;
    ellipsis.type = TYPE_SYMBOL;
    ellipsis.data = "..";
    type = PROCEDURE_LAMBDA;
...
Value* operator_let(List* arguments, List* environment)
{
    ...
    var_def_list = (List*)var_defs->data;
    variable_names.length = 0; variable_names.first = NULL; variable_names.last = NULL;
    initial_values.length = 0; initial_values.first = NULL; initial_values.last = NULL;
    current = var_def_list->first;

    for (i = 0; i < var_def_list->length; i++) {
    ...
    pseudo_call.length = 0; pseudo_call.first = NULL; pseudo_call.last = NULL;
...
Value* operator_cond(List* arguments, List* environment)
{
	...
	clause = (List*)current_clause->value->data;
	if (clause->length < 2) 
	...
	if (!(result->type == TYPE_SYMBOL && !strcmp(result->data, "NIL"))) {
	    prog_exp.length = 0; prog_exp.first = NULL; prog_exp.last = NULL;
