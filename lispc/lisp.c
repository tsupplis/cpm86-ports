...
/* Aztec's stdio.h does not declare fgets, so it would default to int */
...
Value* environment_lookup(List* environment, char* name)
{
    ...
    errorstring = (char*)malloc(sizeof(char) * (strlen(name) + 30));
...
Value* apply(Procedure* procedure, List* arguments, List* environment)
{
	...
	required_args = procedure->free_variables->length - 1;
	...
	if (arguments->length < required_args)
	    ...
	    rest_arg_node = (Node*)malloc(sizeof(Node));
    ...
    new_environment  = alloc_binding_list(procedure->free_variables, arguments);
...
Value* eval(Value* expression, List* environment)
{
	...
	evaluated_list.length = 0; evaluated_list.first = NULL; evaluated_list.last = NULL;
	if (list->length == 0) 
	    return expression;

	/* If the first element is a symbol, it might refer to an operator */
	if (list->first->value->type == TYPE_SYMBOL) { 
	    symbol = (char*)list->first->value->data;
	...
	for (i = 0; i < list->length; i++) {
	    element = eval(current->value, environment);
	...
	if (evaluated_list.first->value->type == TYPE_PROCEDURE) {
	    arguments = list_pop(&evaluated_list);
...
void test_repl()
{
    ...
    while(1) {
	printf("LISP> ");
	input=fgets(expression, sizeof(expression), stdin);
    if(!input || !stricmp(".quit\r\n",input) || !stricmp(".quit\n",input)) {
        return;
    }
