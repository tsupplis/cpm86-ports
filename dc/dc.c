...
usage(void)
{
	printf("dc - arbitrary precision desk calculator\n");
	printf("usage: dc [-h] [file]\n\n");
	printf("Reverse Polish: operands first, then the operator.\n");
	printf("  3 4 + p     prints 7      6 7 * p     prints 42\n\n");
	printf("num      push number (0-9A-F, _ prefix negates, . allowed)\n");
	printf("+ - * / %% ^  pop two values, push the result\n");
	printf("v        square root      Z  length      X  scale factor\n");
	printf("p P f    print top / top as string / whole stack\n");
	printf("d c z    duplicate top / clear stack / push stack depth\n");
	printf("sX lX    store to / load from register X\n");
	printf("SX LX    push to / pop from register X used as a stack\n");
	printf("[...]    push string      x  execute top of stack\n");
	printf("<X >X =X execute register X if the comparison holds\n");
	printf("i I o O  set/get input base, set/get output base\n");
	printf("k K      set/get scale factor (fraction digits kept)\n");
	printf("?        read and execute one line of input\n");
	printf("q Q      quit / pop that many execution levels\n");
	exit(0);
}
...
command(void)
{
	...
	case '=':
		return(cond(NE));
	default:
		/* no command processor to shell out to under CP/M-86 */
		while((c = readc()) != '\n')
			;
		printf("!\n");
		return(0);
